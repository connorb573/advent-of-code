...
void getDeletableDirs(std::shared_ptr<Node> node, int space_needed, std::shared_ptr<std::vector<int>> size_ptr){
    //Check children first
    for(auto& child : node->children){
        getDeletableDirs(child, space_needed, size_ptr);
    }

    if(node->type == "dir"){
        if(node->size > space_needed){
            size_ptr->push_back(node->size);
        }
    }
}
...
int main() {
    ...
    auto size_ptr = std::make_shared<std::vector<int>>(sizes);

    int unused_space = 70000000 - curr->size;
    int space_needed = 30000000 - unused_space;

    getDeletableDirs(curr, space_needed, size_ptr);

    int minimum_size = *std::min_element(size_ptr->begin(), size_ptr->end());

    std::cout << minimum_size << std::endl;
}